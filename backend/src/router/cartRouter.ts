...
router.post('/addItemInShoppingCart', async function (req: Request, res: Response) {
    console.log(`addItemInShoppingCart`)
    let resultObj:ReturnObj = {
        code: 0,
        message: "",
        result: null
    ...
    const result = await cartService.addItemInShoppingCart(cartId,itemId, amount, userId);
    if (result == null) {
        resultObj.code = ReturnStatusCode.NO_DATA_FOUND
        resultObj.message = "no data found"
        res.status(200).json(resultObj)
