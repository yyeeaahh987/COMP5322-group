    ...
    try {
        ...
        if (cartRow.length > 0) {
            ...
            for (let i = 0; i < cartRow.length; i++) {
                sql = `select STOCK from ITEM i where ITEM_ID = ?`
                let [orginStockRows]: any = await connection.promise().query(sql, [cartRow[i].ITEM_ID]);
                let originStockAmt = orginStockRows[0].STOCK
                if(originStockAmt < cartRow[i].AMOUNT){
                    return null
                }
                ...
                let newStockAmt = orginStockRows[0].STOCK -  cartRow[i].AMOUNT 
                sql = `update ITEM set STOCK = ? where ITEM_ID = ?`
                let [itemStockRows]: any = await connection.promise().query(sql, [newStockAmt, cartRow[i].ITEM_ID]);
                totalOrderAmt = totalOrderAmt + (cartRow[i].PRICE * cartRow[i].AMOUNT)
            }
            sql = `update USER_ORDER SET PRICE = ? WHERE ORDER_ID = ?`
            const [updateUserOrderRows]: any = await connection.promise().query(sql, [totalOrderAmt, orderId]);
        }
